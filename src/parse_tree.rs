    ...
    pub fn from_str(str: &str) -> Result<Self, Error> {
        ...
        for c in str.chars() {
            match c {
                ...
                c if c.is_digit(10) => {
                    expr_stack.push(Expr::Int(c.to_digit(10).unwrap() as i64))
                }
                c if c.is_ascii_lowercase() => {
                    expr_stack.push(Expr::Var(c))
                }
                '+' => {
                    let Some(right) = expr_stack.pop() else { return Err(Error::Syntax); };
                    let Some(left) = expr_stack.pop() else { return Err(Error::Syntax); };
                ...
                '=' => {
                    ...
                    let stmt = match left {
                        Expr::Var(v) => Stmt::Set(v, right),
                        _ => return Err(Error::Semantic)
                    };

                    for expr in &expr_stack {
                        stmt_list.push(Stmt::Expr(expr.clone()));
                    }
                    expr_stack.clear();
